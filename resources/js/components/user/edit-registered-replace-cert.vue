<template>
 <div class="container">
  ...
  >
    <v-card-title class="text-h6 font-weight-regular justify-space-between">
      <span><h3>Edit certificate replace registration</h3></span>
      <v-avatar
 ...
 <v-text-field
          ...
          ></v-text-field>
          <v-text-field
            label="Product name & net weight"
            type="text"
            placeholder="eg. cake (1.4kg), sugar (0.6kg)"
 ...
 export default {
  components:{paystack},
    data(){
      return{
         ...
         UPDATE_REGISTERED_REPLACE_CERT:updateApi.UPDATE_REGISTERED_REPLACE_CERT,
         VIEW_REGISTERED_REPLACE_CERT:getApi.VIEW_REGISTERED_REPLACE_CERT,
  ...
  methods:{
    ...
    sendEmail(val){
     ...
     let body=`
      <div>
      <p>Dear ${this.user.firstname}, we are pleased to inform you that your REPLACE CERTIFICATE order has been edited.</p>
      <p></p>
    ...
    submitDetails(){
       ...
       //this.axios.post('/api/register-product',doc,Config).
           this.UPDATE_REGISTERED_REPLACE_CERT(this.$route.params.id,form)
           .then(r=>{
     ...
     created(){
      ...
      this.VIEW_REGISTERED_REPLACE_CERT(this.$route.params.id).then(r=>{
        this.data=r.data;
