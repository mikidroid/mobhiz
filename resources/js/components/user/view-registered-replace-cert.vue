...
<v-btn
   small
   :to="'/user/edit-registered-replace-cert/'+data.id"
   color="blue"
   outlined>
 ...
 <v-btn
   small
   color="blue"
   :href="'/storage/register-replaceCert/'+data.documents"
...
<table class="table">
  <tbody>
    ...
    <tr>
      <td class="font-weight-bold">Product name(s)</td>
...
import sendE from '../config/send-email.js';
 ...
 export default {
     ...
     created(){
       this.VIEW_REGISTERED_REPLACE_CERT(this.$route.params.id)
       .then(r=>{
         this.data=r.data;
       })
       .catch(e=>{
